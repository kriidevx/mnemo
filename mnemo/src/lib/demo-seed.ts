...
// Demo seed — the exact PRD demo narrative (Aarav quote, Swapna launch, SP Road, Khmer Kitchen).
...
export const DEMO_ATOMS: MemoryAtom[] = [
  atom({
    type: 'promise',
    content: 'Send revised Aquastar quote to Aarav — ₹4,200',
    entities: { people: ['Aarav'], orgs: ['Aquastar'], money: { amount: 4200, currency: 'INR' }, dates: [new Date(now + DAY).toISOString().slice(0, 10)] },
    source: 'voice_input',
    language_detected: 'mixed',
    confidence: 0.92,
    created_at: now - 1 * DAY,
    last_recalled: now - 1 * DAY,
    tags: ['aquastar', 'invoicing', 'aarav'],
  }),
  atom({
    type: 'decision',
    content: 'Launch pushed to August — Swapna confirmed',
    entities: { people: ['Swapna'], orgs: [], money: null, dates: ['2026-08-01'] },
    source: 'voice_input',
    confidence: 0.95,
  ...
  atom({
    type: 'financial',
    content: 'Khmer Kitchen pending payment to us — ₹4,200 outstanding',
    entities: { people: [], orgs: ['Khmer Kitchen'], money: { amount: 4200, currency: 'INR' }, dates: [] },
    source: 'whatsapp_notification',
    confidence: 0.88,
    created_at: now - 4 * DAY,
    last_recalled: now - 4 * DAY,
    tags: ['payments', 'khmer-kitchen'],
  }),
  atom({
    type: 'financial',
    content: 'I owe Aarav ₹460 for the SP Road components run',
    entities: { people: ['Aarav'], orgs: [], money: { amount: 460, currency: 'INR' }, dates: [] },
    source: 'manual',
    language_detected: 'en',
    confidence: 0.9,
  ...
  atom({
    type: 'task',
    content: 'Follow up with Loco Lane on bottle pricing',
    entities: { people: [], orgs: ['Loco Lane'], money: null, dates: [] },
    source: 'calendar',
    confidence: 0.75,
    created_at: now - 2 * DAY,
    last_recalled: now - 2 * DAY,
    tags: ['loco-lane', 'pricing'],
  }),
  ...
  atom({
    ...
    entities: { people: ['Swapna'], orgs: [], money: null, dates: [] },
    source: 'whatsapp_notification',
    confidence: 0.62,
